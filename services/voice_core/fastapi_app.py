...
app = FastAPI(
    title="Xiaole Voice Core API",
    description="XiaoLe ASR + LLM + TTS service.",
    version="0.1.0",
)
...
def _enterprise_enabled() -> bool:
    return _parse_bool(_env("XIAOZHI_ENTERPRISE_KB_ENABLED"), True)


def _enterprise_llm_enabled() -> bool:
    return _parse_bool(_env("XIAOZHI_ENTERPRISE_KB_LLM_ENABLED"), False)


def _get_enterprise_service():
    global _ENTERPRISE_SERVICE
    if _ENTERPRISE_SERVICE is None:
        import enterprise_service

        _ENTERPRISE_SERVICE = enterprise_service
...
def _call_llm(session_id: str, input_text: str, history: Any = None) -> str:
    ...
    for key in ["top_p", "frequency_penalty"]:
...
def _call_enterprise_llm(
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "你是小乐语音助手，现在同时具备企业服务台知识库能力。"
                "请优先依据提供的企业知识库内容回答员工问题。"
                "不要编造公司政策、审批结果、金额标准或承诺时效；"
                "如果知识库没有明确依据，请说明需要提交工单或转人工确认。"
                "回答要适合语音播报：先给结论，再列关键步骤，最后给知识库来源。"
            ),
        },
        *_normalize_history(history)[-8:],
        {
            "role": "user",
            "content": (
                f"用户问题：{input_text}\n\n"
                f"企业知识库资料：\n{retrieval.get('context', '')}\n\n"
                "请用中文回答，并在末尾列出来源标题和编号。"
            ),
        },
    ]
    request_params: dict[str, Any] = {
    ...
    answer = response.choices[0].message.content.strip()
    if not answer:
        raise RuntimeError("LLM returned an empty enterprise answer.")
    return answer


def _tts_speech_text(text: str, max_chars: int = 320) -> str:
...
def _quick_local_answer(input_text: str) -> str | None:
    normalized = re.sub(r"\s+", "", str(input_text or "").lower())
    if not normalized:
        return None
    greeting_words = ["你好", "您好", "hello", "hi", "嗨"]
    identity_words = ["你是谁", "你叫什么", "介绍一下你自己", "自我介绍", "叫什么名字"]
    if any(word in normalized for word in greeting_words) and len(normalized) <= 12:
        return "你好，我是小乐，你的智能语音助手。有什么可以帮你的吗？"
    if any(word in normalized for word in identity_words):
        return "我是小乐，一个可以文字聊天、语音问答，并能查询企业服务台知识库的智能语音助手。"
    return None


def _answer_text_sync(
    session_id: str, input_text: str, history: Any = None
) -> dict[str, Any]:
    ...
    if _enterprise_enabled():
        try:
            service = _get_enterprise_service()
            retrieval = service.retrieve(input_text)
            if retrieval.get("matched"):
                if _enterprise_llm_enabled():
                    try:
                        answer = _call_enterprise_llm(
                            session_id, input_text, history, retrieval
                        )
                    except Exception:
                        answer = service.build_fallback_answer(input_text, retrieval)
                else:
                    answer = service.build_fallback_answer(input_text, retrieval)
                metadata["source"] = "enterprise_knowledge_base"
                metadata["enterprise"] = retrieval
                return {"answer_text": answer, "metadata": metadata}
        except Exception as exc:
            metadata["enterprise_error"] = str(exc)

    quick_answer = _quick_local_answer(input_text)
    if quick_answer:
        metadata["source"] = "local_quick_reply"
        return {"answer_text": quick_answer, "metadata": metadata}

    return {
        "answer_text": _call_llm(session_id, input_text, history),
        "metadata": metadata,
    }
...
async def _answer_text(
    ...
    return await asyncio.to_thread(_answer_text_sync, session_id, input_text, history)
        ...
        payload.update(
            {
                ...
                "voice_rate": form.get("voice_rate"),
                "voice_pitch": form.get("voice_pitch"),
                "voice_volume": form.get("voice_volume"),
...
async def health():
    ...
    try:
        ...
        if _enterprise_enabled():
            try:
                details["enterprise_knowledge_base"] = _get_enterprise_service().stats()
            except Exception as exc:
                details["enterprise_knowledge_base_error"] = str(exc)
    except Exception as exc:
...
async def ask(request: Request):
    ...
    timings_ms["parse_request"] = int((time.perf_counter() - started_at) * 1000)
    ...
    if payload.get("audio_bytes"):
        try:
            ...
            timings_ms["asr"] = int((time.perf_counter() - asr_started_at) * 1000)
        except Exception as exc:
    ...
    try:
        ...
        answer_result = await _answer_text(session_id, input_text, payload.get("history"))
        answer_text = answer_result["answer_text"]
        answer_metadata = answer_result.get("metadata") or {}
        timings_ms["answer"] = int((time.perf_counter() - llm_started_at) * 1000)
    except Exception as exc:
    ...
    if payload.get("return_audio"):
        try:
            ...
            audio_file = await _synthesize_audio(
                session_id,
                _tts_speech_text(answer_text),
                payload.get("voice"),
                payload.get("voice_rate"),
                payload.get("voice_pitch"),
                payload.get("voice_volume"),
            )
            ...
            timings_ms["tts"] = int((time.perf_counter() - tts_started_at) * 1000)
        except Exception as exc:
    ...
    return JSONResponse(
        {
            ...
            "answer_source": answer_metadata.get("source", "llm"),
            "enterprise": answer_metadata.get("enterprise"),
            "audio_url": audio_url,
...
async def tts(request: Request):
    ...
    text = _tts_speech_text(str(body.get("text") or "").strip())
    if not text:
        raise HTTPException(status_code=400, detail="Provide text for TTS.")
    ...
    try:
        audio_file = await _synthesize_audio(
            ...
            body.get("voice"),
            body.get("voice_rate"),
            body.get("voice_pitch"),
            body.get("voice_volume"),
        )
    except Exception as exc:
