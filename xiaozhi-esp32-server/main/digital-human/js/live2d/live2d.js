...
class Live2DManager {
    ...
    setLive2DStageVisible(isVisible) {
    ...
    async getMmdViewer() {
        if (this.mmdViewer) {
            return this.mmdViewer;
        }

        if (!this.pendingMmdModule) {
            this.pendingMmdModule = import('../mmd/mmd-viewer.js?v=0225');
        }

        const { MMDViewer } = await this.pendingMmdModule;
        const container = document.getElementById('mmd-stage');
        this.mmdViewer = new MMDViewer(container);
        return this.mmdViewer;
    }

    async showMmdModel(modelName) {
        ...
        try {
            ...
            this.stopTalking();
            this.setLive2DStageVisible(false);

            const viewer = await this.getMmdViewer();
            const modelPath = this.getBasePath() + 'resources/hutao_mmd/kqsht/hutao.pmx';
            await viewer.init(modelPath);

            this.currentModelName = modelName;
            localStorage.setItem('live2dModel', modelName);
            this.updateModelSelect(modelName);
            return true;
        } catch (error) {
            console.error('加载 MMD 模型失败:', error);
            this.setLive2DStageVisible(true);
            return false;
        } finally {
            if (app) {
    ...
    async initializeLive2D() {
        try {
            ...
            let savedModelName = localStorage.getItem('live2dModel') || 'hiyori_pro_zh';
            if (this.isMmdModel(savedModelName)) {
                const mmdLoaded = await this.showMmdModel(savedModelName);
                if (mmdLoaded) {
                    return;
                }
                savedModelName = 'hiyori_pro_zh';
            }

            // 加载 Live2D 模型 - 动态检测当前目录，适配不同环境
            const basePath = this.getBasePath();
            const modelFileMap = this.getLive2DModelFileMap();
            if (!modelFileMap[savedModelName]) {
                savedModelName = 'hiyori_pro_zh';
            }

            this.hideMmdModel();
            const modelFileName = modelFileMap[savedModelName];
            ...
            // 更新下拉框显示
            this.updateModelSelect(savedModelName);
     ...
     */
    startTalking() {
        if (this.isMmdModel(this.currentModelName)) {
            this.isTalking = true;
            if (this.mmdViewer) {
                this.mmdViewer.startTalking();
            }
            return;
        }
    ...
    triggerEmotionAction(emotion) {
        if (this.isMmdModel(this.currentModelName)) return;
        if (!this.live2dModel) return;
    ...
    async switchModel(modelName) {
        try {
            if (this.isMmdModel(modelName)) {
                return await this.showMmdModel(modelName);
            }
