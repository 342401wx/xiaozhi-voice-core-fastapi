...
class UIController {
    ...
    init() {
        ...
        audioRecorder.onRecordingStart = (seconds) => {
            ...
            this.updateMicLevelPanel(true);
        };
        audioRecorder.onRecordingStop = () => {
            this.updateMicLevelPanel(false);
        };
    ...
    updateAudioStats() {
        ...
        this.updateMicLevelPanel();

        const wsHandler = getWebSocketHandler();
        const statusText = document.getElementById('connectionStatus')?.textContent;
        if (statusText && statusText !== '离线' && !wsHandler.isConnected()) {
            this.updateConnectionUI(false);
            this.updateDialButton(false);
        }
    }

    updateMicLevelPanel(forceVisible = null) {
        const panel = document.getElementById('micLevelPanel');
        ...
        const audioRecorder = getAudioRecorder();
        const inputStats = audioRecorder.getInputStats ? audioRecorder.getInputStats() : { isRecording: false, level: 0 };
        const isVisible = forceVisible === null ? inputStats.isRecording : forceVisible;

        panel.hidden = !isVisible;
        if (!isVisible) {
            fill.style.width = '0%';
            label.textContent = '麦克风 0%';
            return;
        }

        const level = Math.max(0, Math.min(1, inputStats.level || 0));
        const percent = Math.round(level * 100);
        fill.style.width = `${percent}%`;
        label.textContent = `麦克风 ${percent}%`;
    }
